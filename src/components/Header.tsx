  ...
  const NavLink = ({ href, children }: { href: string; children: React.ReactNode }) => {
    ...
    return (
      <Link href={href} className="relative group text-sm font-medium">
        <span className={`transition-opacity duration-200 ${active ? "opacity-100" : "opacity-50 group-hover:opacity-100"}`}>
          {children}
        </span>
    ...
    <>
      ...
      >
        <div className={`transition-all duration-300 ${
          scrolled
            ? "bg-white border-b border-gray-100 shadow-[0_2px_20px_rgba(0,0,0,0.06)]"
            : "bg-white"
        }`}>
          <div className="max-w-7xl mx-auto px-6 md:px-10 flex items-center justify-between h-[68px]">
            ...
            <Link
              href="/"
              className="font-semibold text-[18px] tracking-tight text-[#1E1E1E] hover:opacity-70 transition-opacity duration-200"
            >
              Roomforia
            </Link>
            ...
            >
              <motion.span
                ...
                className="block w-5 h-[1.5px] bg-[#1E1E1E] origin-center"
              />
              <motion.span
                animate={{ opacity: open ? 0 : 1, scaleX: open ? 0 : 1 }}
                transition={{ duration: 0.2 }}
                className="block w-5 h-[1.5px] bg-[#1E1E1E]"
              />
              <motion.span
                animate={{ rotate: open ? -45 : 0, y: open ? -7 : 0 }}
                transition={{ duration: 0.3 }}
            ...
            >
              ...
              >
                <Link
                  href="/partners"
                  onClick={() => setOpen(false)}
                  className="w-full py-3.5 rounded-full border-2 border-[#855dda] text-[#855dda] text-center font-medium text-sm"
                >
                  Стать партнёром
                </Link>
                <a
