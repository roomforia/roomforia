      ...
      >
        ...
        <div className="relative px-8 md:px-16 lg:px-20 py-14 md:py-28">
          ...
          <div className="md:hidden mb-8">
            <p className="text-[32px] font-semibold tracking-tight text-white leading-[1.1] mb-1">Roomforia —</p>
            <p className="text-[32px] font-semibold tracking-tight text-[#d66501] leading-[1.1]">платформа для роста</p>
          </div>

          {/* Десктоп */}
          <div className="hidden md:block mb-12">
            <div className="flex flex-wrap items-end overflow-hidden mb-1">
              {"Roomforia —".split("").map((char, i) => (
                <motion.span key={i}
            ...
            <div className="flex flex-wrap items-end overflow-hidden">
              {"платформа для роста".split("").map((char, i) => (
                <motion.span key={i}
                  initial={{ opacity: 0, y: 60, filter: "blur(8px)" }}
                  whileInView={{ opacity: 1, y: 0, filter: "blur(0px)" }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1], delay: (11 + i) * 0.04 }}
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 md:gap-10">
            ...
            >
              AI работает на продажи. Присоединяйтесь к платформе, которая меняет то, как люди покупают интерьер.
            </motion.p>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <span className="text-white/25 text-sm">Связываемся в течение дня</span>
            </motion.div>
          </div>
          ...
          <div className="mt-6 md:mt-8 grid grid-cols-3 gap-4 md:gap-8">
            {[
              ...
              { num: "3", label: "страны присутствия" },
              { num: "6+", label: "направлений в команде" },
            ].map((stat, i) => (
              <motion.div key={i}
